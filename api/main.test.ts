...
Deno.test("Testing API station", async () => {
  ...
  const response = await fetch(
    `http://localhost:${PORT}/api/station/${config.stations[0].key}`,
  );
  const body = await response.json();
...
// TODO: not yet implemented - need to mock response 
Deno.test("Testing API day month year", async () => {
  const controller = new AbortController();
  const signal = controller.signal;
  const api = app.listen({ port: PORT, signal });

  const response = await fetch(
    `http://localhost:${PORT}/api/station/${
      config.stations[0].key
